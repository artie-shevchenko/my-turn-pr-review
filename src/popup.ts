...
async function populate() {
  ...
  const repos: Repo[] = (await getRepos()).filter((r) => r.monitoringEnabled
      // ignore repos which are not synced yet (sync is definitely in progress):
      && repoStateByFullName.get(r.fullName()));

  const reposWithProblems =
      repos.filter((r) => {
        const repoState = repoStateByFullName.get(r.fullName());
        console.assert(repoState.lastSyncResult, "No last sync? " + repoState.lastSyncResult);
  ...
  repoListSection.innerHTML =
      ...
      repos
          ...
          // repoWarnSection:
          .filter((r) => !repoStateByFullName.get(r.fullName()).lastSyncResult.errorMsg)
          .map((r) => r.fullName())
          .join(", ");
  ...
  const reviewsRequested = repos
      .flatMap((repo) => {
        ...
        if (!repoState.lastSuccessfulSyncResult || !repoState.lastSuccessfulSyncResult.isRecent()) {
